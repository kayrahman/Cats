    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `chapter` (`id` INTEGER, `revelationPlace` TEXT, `revelationOrder` INTEGER, `bismillahPre` INTEGER, `nameComplex` TEXT, `nameArabic` TEXT, `versesCount` INTEGER, `translatedName` TEXT, PRIMARY KEY (`id`))');
...
class _$ChapterDao extends ChapterDao {
  _$ChapterDao(
    ...
    this.changeListener,
            ...
            (ChapterModel item) => <String, Object?>{
                  ...
                  'bismillahPre': item.bismillahPre == null
                      ? null
                      : (item.bismillahPre! ? 1 : 0),
                  'nameComplex': item.nameComplex,
                  'nameArabic': item.nameArabic,
                  'versesCount': item.versesCount,
                  'translatedName':
                      _translatedNameEntityConverter.encode(item.translatedName)
                });
  ...
  Future<void> insertChapter(ChapterModel chapter) async {
    await _chapterModelInsertionAdapter.insert(
        chapter, OnConflictStrategy.abort);
  }
}

// ignore_for_file: unused_element
final _translatedNameEntityConverter = TranslatedNameEntityConverter();
